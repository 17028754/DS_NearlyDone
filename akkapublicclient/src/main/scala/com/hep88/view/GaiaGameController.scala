...
class GaiaGameController(
  ...
  var clientRef: ActorRef[GameClient.Command] = ClientRef.clientRef.get
  var clientName: String = ClientRef.clientName
  var ownRef: ActorRef[GameClient.Command] = ClientRef.ownRef.get
  ...
  tetris.onKeyPressed = (e: KeyEvent) => {
    ...
    if (e.code == KeyCode.P) {
      ...
      if (!pause && !gameOver) {
        Client.userRef ! GameClient.OwnPause(clientRef)
        pauseSoundEffect()
        timer.stop
      ...
      // Unpause game and tell opponent to unpause also
        ...
        if (pause && !gameOver) {
          Client.userRef ! GameClient.OwnUnpause(clientRef)
          pauseSoundEffect()
          timer.start
          showPaused.setText("")
          pause = false
  ...
  def updateScore(): Unit = {
    ...
    Client.userRef ! GameClient.PutScore(clientRef, scores.toString)
  }
  ...
  val timer: AnimationTimer = AnimationTimer(t => {
    ...
    if (game.nextPiece.isEmpty) {
      ...
      Client.userRef ! GameClient.TellNextPiece(clientRef, game.nextPiece)
    ...
    if (game.currentPiece.isEmpty) {
      ...
      Client.userRef ! GameClient.ClearNextPiece(clientRef)
      game.nextPiece = List()
      ...
      if (game.checkGameOver()) {
        ...
        Client.userRef ! GameClient.SendGameOver(clientRef, gameOver, game.scores)
      }
      ...
      if (gameOver == true && gameOverOpponent == true){
        ...
        showPaused.setText("Game Over!")
        Client.userRef ! GameClient.SendGameOverLast(clientRef, game.scores)
      }
      ...
      Client.userRef ! GameClient.CurrentBoardPiece(clientRef, game.currentPiece, game.tetraminoes.currentX, game.tetraminoes.currentY)
    }
    ...
    if (leftPressed) {
      ...
      leftPressed = false
      Client.userRef ! GameClient.RefreshEnemyBoard(clientRef, game.gameBoard.board)
      Client.userRef ! GameClient.PaintEnemyBoardPiece(clientRef, game.currentPiece, game.tetraminoes.currentX, game.tetraminoes.currentY)
    }
    if (rightPressed) {
      // go right
      game.moveSet(1,0)
    ...
    if (upPressed) {
      ...
      if (game.isRotatable()) {
        ...
        paintPieceToBoard(game.currentPiece, game.tetraminoes.currentX, game.tetraminoes.currentY)
        Client.userRef ! GameClient.ClearEnemyBoardPiece(clientRef, game.currentPiece,game.tetraminoes.currentX,game.tetraminoes.currentY, game.gameBoard.board)
        Client.userRef ! GameClient.PaintEnemyBoardPiece(clientRef, game.currentPiece, game.tetraminoes.currentX, game.tetraminoes.currentY)
      }
      else
